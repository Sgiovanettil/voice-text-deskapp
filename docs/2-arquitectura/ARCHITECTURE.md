...
- **Comandos** (frontend → core): `get_settings`, `set_settings`, `set_api_key`, `test_provider`, `list_models`, `get_usage`, `reset_usage`, `get_usage_rates`, `get_app_state`. Validación en el borde; errores mapeados a `{ code, error_key }`.
