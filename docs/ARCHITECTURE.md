...
### 4.3 `audio/`
- Captura con `cpal`, dispositivo por defecto del sistema (selección de dispositivo: v1.x).
- Conversión a **16 kHz mono f32→i16** (resampling con `rubato` o equivalente), buffer en memoria (`Vec<i16>`, límite 120 s ≈ 3,8 MB).
...
`ProviderRegistry`: mapa `id → factory`; resuelve según `config.stt.provider`. Registrar un proveedor nuevo = implementar el trait + una línea en el catálogo del registry (RNF-08). Cuando exista el segundo proveedor, `providers/*` migra a crates propios (PRD §17.2).

### 4.5 `providers/openai/`
- Endpoint `POST /v1/audio/transcriptions` (multipart WAV).
- **Modelo por defecto: `gpt-4o-mini-transcribe`** — mejor relación latencia/costo/calidad que `whisper-1`; `gpt-4o-transcribe` y `whisper-1` seleccionables en settings. *(Supuesto por ratificar, PRD §17.3.)*
...
### 4.6 `delivery/`
Dos modos (RF-06): `insert` (texto en la app activa) y `clipboard` (solo copiar). El mecanismo de `insert` es la decisión **ADR-005** — ver evaluación y propuesta en [`adr/0005`](adr/0005-mecanismo-insercion-texto.md). Resumen de la propuesta:

- **Primario:** clipboard + orden de pegado sintética (Ctrl+V) con **guardado/restauración del clipboard** (solo texto en MVP).
- **Perfiles de pegado por app:** combinación configurable (los terminales usan Ctrl+Shift+V); detección por clase de ventana en X11/Windows con tabla editable (en Wayland la identificación de la ventana activa es limitada: perfil por defecto + override manual).
...
Pendiente de **spike de validación** (matriz de compatibilidad editor/terminal/navegador en Windows, X11 y Wayland GNOME/KDE) antes de marcar el ADR como Aceptada.
